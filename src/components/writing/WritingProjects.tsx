...
import WritingSprint from '@/components/writing/WritingSprint';
import OutlineBuilder from '@/components/writing/OutlineBuilder';
import WritingHeatmap from '@/components/writing/WritingHeatmap';
import RevisionChecklist from '@/components/writing/RevisionChecklist';
import WritingScratchpad from '@/components/writing/WritingScratchpad';
import RandomPromptStarter from '@/components/writing/RandomPromptStarter';
import JustOneParagraph from '@/components/writing/JustOneParagraph';
...
function WritingDetail({ project, onBack }: { project: WritingProject; onBack: () => void }) {
  ...
  const [paragraphSection, setParagraphSection] = useState<string | undefined>();
  const [activeTab, setActiveTab] = useState<'overview' | 'outline' | 'tools'>('overview');
  ...
  const updateWordCount = async () => {
    ...
    await db.writingProjects.update(wp.id, { currentWordCount: newWC, wordCountHistory: history, updatedAt: now() } as any);
    refresh();
  };
    ...
    <div className="page-content" style={{ padding: '24px 28px', maxWidth: 1000 }}>
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn-primary" onClick={() => setShowSprint(true)} style={{ fontSize: 13 }}>
            Sprint
          </button>
          <button className="btn-ghost" onClick={() => { setParagraphSection(undefined); setShowParagraph(true); }} style={{ fontSize: 13 }}>
            Just 1 Paragraph
          </button>
          {wp.editorUrl && (
            <a href={wp.editorUrl} target="_blank" rel="noopener noreferrer" className="btn-ghost" style={{ textDecoration: 'none', fontSize: 13 }}>
              Editor ↗
            </a>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: '2px solid var(--border-light)' }}>
        {(['overview', 'outline', 'tools'] as const).map(tab => (
          <button
            key={tab}
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            {/* Activity Heatmap */}
            <WritingHeatmap history={wp.wordCountHistory} />

            {/* Revision Checklist */}
            <RevisionChecklist project={wp} onRefresh={refresh} />
          </div>
      ...
      {activeTab === 'outline' && (
        <OutlineBuilder project={wp} onRefresh={refresh} />
      )}

      {activeTab === 'tools' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <RandomPromptStarter genre={wp.genre} outlineSections={outlineSections} />
            <WritingScratchpad project={wp} onRefresh={refresh} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {/* Quick actions card */}
            <div className="card" style={{ padding: 20 }}>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 16, marginBottom: 14 }}>ADHD Writing Tools</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button className="btn-ghost" onClick={() => setShowSprint(true)} style={{
                  width: '100%', textAlign: 'left', padding: '12px 14px',
                  background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)',
                }}>
                  <span style={{ fontSize: 14 }}>Sprint Mode</span>
                  <br />
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Timed writing session with word tracking</span>
                </button>
                <button className="btn-ghost" onClick={() => { setParagraphSection(undefined); setShowParagraph(true); }} style={{
                  width: '100%', textAlign: 'left', padding: '12px 14px',
                  background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)',
                }}>
                  <span style={{ fontSize: 14 }}>Just One Paragraph</span>
                  <br />
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>10 minutes. One paragraph. Low pressure.</span>
                </button>
                {outlineSections.length > 0 && (
                  <button className="btn-ghost" onClick={() => {
                    ...
                    <span style={{ fontSize: 14 }}>Random Section Focus</span>
                    <br />
                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Pick a random outline section to work on</span>
                  </button>
                )}
              </div>
            </div>
      ...
      <WritingSprint open={showSprint} onClose={() => setShowSprint(false)} project={wp} />
      <JustOneParagraph open={showParagraph} onClose={() => setShowParagraph(false)} sectionTitle={paragraphSection} />
